...
void Character::collision(float t, LineSegment segments[10]) {
    if (jump_segment == NULL || t - jump.t0 < 0.1f)
        return;
    ...
    for (usize i = 0; i < 10; ++i) {
        ...
        bool intersects = physics::intersect_cohen(copy_segment, dude);
...
void Character::sample(float t) {
    ...
    if (current_segment != NULL) {
        ...
        if (next != current_segment) {
            f32 dist = sqrt(next->x.d * next->x.d + next->y.d * next->y.d);
            ...
            printf("(%f,%f) -> (%f,%f) %f m (%f sec)\n", current_segment->x.p,
                   current_segment->y.p, next->x.p + next->x.d,
                   next->y.p + next->y.d, dist, dist / VELOCITY);
...
void Character::update(float t, const uint8_t* keyboard_state) {
    if (keyboard_state[SDL_SCANCODE_SPACE] && jump_segment == NULL) {
