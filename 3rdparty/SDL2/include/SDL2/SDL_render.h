 ...
 * plane. \param Uplane    The raw pixel data for the U plane. \param Upitch    The number of bytes between rows of pixel data for the U
 * plane. \param Vplane    The raw pixel data for the V plane. \param Vpitch    The number of bytes between rows of pixel data for the V
 * plane.
 *
 *  \return 0 on success, or -1 if the texture is not valid.
 *
 ...
 * If the actual output resolution doesn't have the same aspect ratio the
 * output rendering will be centered within the output display.
 *
 ...
 * applied to dstrect, rotating it in a clockwise direction \param center   A
 * pointer to a point indicating the point around which dstrect will be rotated
 * (if NULL, rotation will be done around dstrect.w/2, dstrect.h/2). \param flip
 * An SDL_RendererFlip value stating which flipping actions should be performed
 * on the texture
 *
 *  \return 0 on success, or -1 on error
 */
 ...
 * width \param texh     A pointer to a float that will be filled with the
 * texture height
 *
